                ...
                @foreach ($thumbs as $thumb)
                  <tr>
                    ...
                    <td>
                      ...
                      <a class="text-black" href="{{ route('dashboard.users.show', $thumb->user) }}">{{ $thumb->user->nickname ?: 'NULL' }}</a>
                    </td>

                    <td>
                      @if ($thumb->thumbable->user)
                        ...
                        <a class="text-black" href="{{ route('dashboard.users.show', $thumb->thumbable->user) }}">{{ $thumb->thumbable->user->nickname ?: 'NULL' }}</a>
                      @else
                    ...
                    <td>
                      @if ($thumb->entity_class === \Modules\Post\Entities\Post::class)
                        <a class="text-black" href="{{ route('dashboard.posts.show', $thumb->entity_id) }}">{{ $thumb->entity_name }}(ID:{{ $thumb->entity_id }})</a>
                      @elseif ($thumb->entity_class === \App\Models\Common\Comment::class)
                        @if ($thumb->entity->entity_class === \Modules\Post\Entities\Post::class)
                          <a class="d-block text-black" href="{{ route('dashboard.posts.show', $thumb->entity->entity_id) }}">{{ $thumb->entity->entity_name }}(ID:{{ $thumb->entity->entity_id }})</a>
                          <a class="text-black" href="{{ route('dashboard.comments.show', $thumb->entity->id) }}">{{ $thumb->entity_name }}(ID:{{ $thumb->entity_id }})</a>
                        @else
